...
dt = 0.1
d = 0.015
def pick_without_back(position, pitch, yaw, gripper_angle, gripper_depth, joints_pub, kinematics_client, offset_z = 0.01,interpolation=False):
    global stop
    if not stop:
        position[2] += offset_z
...
def pick(position, pitch, yaw, gripper_angle, gripper_depth, joints_pub, kinematics_client,offset_z = 0.03, interpolation=False):
    global stop
    if pick_without_back(position, pitch, yaw, gripper_angle, gripper_depth, joints_pub, kinematics_client,offset_z,interpolation):
        if not stop:
            if interpolation:
                ...
                if chassis_type == 'Slide_Rails':
                    ...
                    msg = set_pose_target([0.17, 0.0, 0.15], 73, [-180.0, 180.0], 1.0)
                res = send_request(kinematics_client, msg)
                servo_data = res.pulse
                set_servo_position(joints_pub, 0.5, ((5, 500),))
                time.sleep(0.02)
...
def place(position, pitch, yaw, gripper_angle, joints_pub, kinematics_client, offset_z = 0.03,interpolation=False):
    global stop
    if not stop:
        position[2] += offset_z
